-- NearPath Tutors — database schema for Supabase (fresh-install version)
--
-- This is the FULL, CURRENT schema with no migration history in it —
-- every table already has every column/constraint the app needs, so
-- there's nothing to "catch up" later. Use this only for a BRAND NEW
-- Supabase project (or one you're rebuilding from scratch).
--
-- Already have an existing project you don't want to lose data on?
-- Use the other file (schema.sql) instead and run just the migration
-- block that matches what you're missing — this fresh-install version
-- will error out on tables/columns that already exist.
--
-- HOW TO RUN: copy this entire file into your Supabase project's
-- SQL Editor (left sidebar → SQL Editor → New query) and click Run.
-- You should see "Success. No rows returned." when it's done.
